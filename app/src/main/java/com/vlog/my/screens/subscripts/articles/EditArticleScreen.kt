...
fun EditArticleScreen(
    ...
    var subScripts: SubScripts? = null
    if (!scriptId.isNullOrEmpty()){
        subScripts = scriptsDataHelper.getUserScriptsById(scriptId)
    }

    // 状态变量
    var name by remember { mutableStateOf(subScripts?.name) }
    var apiKey by remember { mutableStateOf(subScripts?.apiKey) }
    var databaseName by remember { mutableStateOf(subScripts?.databaseName) }
    ...
    var isTyped by remember {
        mutableIntStateOf(subScripts.isTyped)
    }
