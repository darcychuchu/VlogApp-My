...
@Composable
fun AddVideoScriptScreen(
    navController: NavController,
    viewModel: AddVideoScriptViewModel = hiltViewModel()
) {
    ...
    val localFilePickerLauncher = rememberLauncherForActivityResult(
        ...
        uris.forEach { uri ->
            val fileName = uri.pathSegments.lastOrNull() ?: "unknown_video_file"
    ...
    Scaffold(
        topBar = {
            CenterAlignedTopAppBar(
                title = { Text("Add New Video Script") },
                navigationIcon = {
        ...
        Column(
            ...
            Button(onClick = { localFilePickerLauncher.launch("video/*") }) { // Changed to video/*
            ...
            Button(
                onClick = {
                    viewModel.saveVideoScript {
                        navController.popBackStack()
                    }
                },
                modifier = Modifier.fillMaxWidth()
            ) {
                Text("Save Video Script")
            }
        }
    }
}
